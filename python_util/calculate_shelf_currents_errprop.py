...
def calculate_shelf_current_data(params, inputDataPath, calculateGasTransferVelocity=False, testPlot=False, verbose=False, outputPath=None, errorProp=None):
    ...
    ekmanErr = errorProp[0][0];
    geostrophicErr = errorProp[0][1];
    stokesErr = errorProp[0][2];
    ...
    for month, year in monthYears:
        ...
        #Convert to np arrays (not np.ma arrays)
        ekmanEast.data[ekmanEast.mask] = np.nan;
        ekmanEast = ekmanEast.data;
        ekmanNorth.data[ekmanNorth.mask] = np.nan;
        ekmanNorth = ekmanNorth.data;
        geostrophicEast.data[geostrophicEast.mask] = np.nan;
        geostrophicEast = geostrophicEast.data;
        geostrophicNorth.data[geostrophicNorth.mask] = np.nan;
        geostrophicNorth = geostrophicNorth.data;
        stokesEast.data[stokesEast.mask] = np.nan;
        stokesEast = stokesEast.data;
        stokesNorth.data[stokesNorth.mask] = np.nan;
        stokesNorth = stokesNorth.data;
        uwnd.data[uwnd.mask] = np.nan;
        uwnd = uwnd.data;
        vwnd.data[vwnd.mask] = np.nan;
        vwnd = vwnd.data;
        hs.data[hs.mask] = np.nan;
        hs = hs.data;
        
        
        #Apply uncertainty to inputs
        ekmanEast = ekmanEast + np.random.normal(0.0, ekmanErr, ekmanEast.shape);
        ekmanNorth = ekmanNorth + np.random.normal(0.0, ekmanErr, ekmanNorth.shape);
        geostrophicEast = ekmanEast + np.random.normal(0.0, geostrophicErr, geostrophicEast.shape);
        geostrophicNorth = geostrophicNorth + np.random.normal(0.0, geostrophicErr, geostrophicNorth.shape);
        stokesEast = stokesEast + np.random.normal(0.0, stokesErr, stokesEast.shape);
        stokesNorth = stokesNorth + np.random.normal(0.0, stokesErr, stokesNorth.shape);
    ...
    if outputPath != None:
        outputFile = path.join(inputDataPath, "current_data", "surface_currents_"+params.paramsetNameErrorOutput+".p");
